...
def run_simulation(params):
    """
    Executes the FEM simulation in NGSolve with the given parameters.
    ...
    GLIA_DISTANCE = params[3] * u.um
    GLIA_WIDTH = params[4] * u.um
    GLIA_COVERAGE = params[5]  # No units, as it is a factor

    angle = float(np.arccos(1 - 2 * GLIA_COVERAGE)) * u.rad  # Convert angle to radians

    # Create geometry and mesh
    geo = create_rusakov_geometry(
        total_size=TOTAL_SIZE, synapse_radius=SYNAPSE_RADIUS,
        cleft_size=CLEFT_SIZE, glia_distance=GLIA_DISTANCE,
        glia_width=GLIA_WIDTH, glial_coverage_angle=angle
    )
    mesh_size = 0.1 * u.um  # Convert the mesh size to micrometers
...
def match_dimensions(simulated_data, experimental_data):
    """Interpolate simulated_data so it has the same length as experimental_data."""
    sim_time = np.linspace(0, 1, simulated_data.shape[0])  # Normalized simulation time
    exp_time = np.linspace(0, 1, len(experimental_data))  # Normalized time of the experimental data
...
# Trim all series to the same size
experimental_data = np.mean(
    [df.iloc[:min_length, 1].values for df in data.values()], axis=0
)

# Initial parameter range
initial_params = [2.0, 0.1, 30e-3, 30e-3, 100e-3, 0.5]
bounds = [(1.5, 2.5), (0.05, 0.2), (20e-3, 40e-3), (20e-3, 50e-3), (80e-3, 150e-3), (0.3, 0.7)]
...
print(f"  4. GLIA_DISTANCE    = {optimized_params[3]} um")
print(f"  5. GLIA_WIDTH       = {optimized_params[4]} um")
print(f"  6. GLIA_COVERAGE    = {optimized_params[5]}")
