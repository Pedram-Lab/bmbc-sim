...
interface. The interface equation is part of the stiffness matrix, which is
inverted using the conjugate gradient method, where the stiffness matrix is
split into a base part and a time-dependent part.
"""
import time
...
b += dt * transmission * (ul - ur) * (vl - vr) * ngs.ds("interface")
...
with ngs.TaskManager():
    ...
    preconditioner = m.mat.CreateSmoother(fes.FreeDofs())
    ...
    for i in range(N_STEPS):
        b.Assemble()
        mstar_inv = ngs.CGSolver(m.mat + b.mat, preconditioner, printrates=False)

        res = - (dt * a.mat * u.vec + b.mat * u.vec)
        u.vec.data += mstar_inv * res
        t += dt
        if t > 0.5:
            transmission.Set(100)
        transmission.Set(100 + t)
        k += 1
        if k == 10:
            us.AddMultiDimComponent(u.vec)
            k = 0
...
Draw(us.components[1], mesh, min=0, max=1, settings=settings)
