...
inverted using the conjugate gradient method.
"""
import time
...
m += dt * D * ngs.grad(ul) * ngs.grad(vl) * ngs.dx("cell:left")
m += dt * D * ngs.grad(ur) * ngs.grad(vr) * ngs.dx("cell:right")
m += dt * transmission * (ul - ur) * (vl - vr) * ngs.ds("interface")
...
with ngs.TaskManager():
    a.Assemble()
    m.Assemble()
    preconditioner = ngs.Preconditioner(m, "local")

    mstar_inv = ngs.CGSolver(m.mat, preconditioner.mat, printrates=False)
    setup_end = time.time()
