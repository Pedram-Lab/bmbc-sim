...
from ecsim.units import LENGTH
...
def create_rusakov_geometry(
    ...
    """
    Create the geometry from [Rusakov 2003]. It consists of two semi-spheric
    ...
    ts = total_size.to(LENGTH).value
    cs = cleft_size.to(LENGTH).value
    sr = synapse_radius.to(LENGTH).value
    gd = glia_distance.to(LENGTH).value
    gw = glia_width.to(LENGTH).value

    gca = glial_coverage_angle.to(u.deg).value
...
geo = create_rusakov_geometry(
    total_size=3 * u.um,
    synapse_radius=0.4 * u.um,
    cleft_size=30 * u.nm,
    glia_distance=30 * u.nm,
    glia_width=0.2 * u.um,
    glial_coverage_angle=0 * u.deg,
)
...
geo = create_rusakov_geometry(
    ...
    glia_distance=30 * u.nm,
    glia_width=0.2 * u.um,
    glial_coverage_angle=150 * u.deg,
)
Draw(geo, clipping=clipping_settings, settings=visualization_settings)
...
geo = create_rusakov_geometry(
    ...
    glia_distance=30 * u.nm,
    glia_width=0.2 * u.um,
    glial_coverage_angle=180 * u.deg,
)
Draw(geo, clipping=clipping_settings, settings=visualization_settings)
