...
ECS_YOUNGS_MODULUS = 0.5 * u.kPa              # Stiffness of ECM gel
ECS_POISSON_RATIO = 0.3                       # Compressibility (0.3 typical for soft tissue)
ECM_CA_COUPLING = 0.1 * u.kPa / (u.mmol / u.L)  # Mechanical coupling: pressure per mM of ECM_Ca
...
sim = bmbcsim.Simulation(
    mesh=tissue_mesh,
    name="tissue_kinetics_with_mechanics",
    result_root="results",
    mechanics=True
)
...
# ================================================================
# Add elasticity to ECS - this is required for the mechanics solver
ecs.add_elasticity(
    youngs_modulus=ECS_YOUNGS_MODULUS,
    poisson_ratio=ECS_POISSON_RATIO
)

# Add elasticity to all cells (cells are stiffer than ECM)
CELL_YOUNGS_MODULUS = 1.0 * u.kPa  # Cells typically stiffer than ECM
CELL_POISSON_RATIO = 0.4           # Cells are more incompressible
for cell in cells:
    cell.add_elasticity(
...
# Use equilibrium ECM_Ca concentration as baseline so no deformation at rest
ecs.add_driving_species(ecm_ca, ECM_CA_COUPLING, baseline=ECM_CA_EQUILIBRIUM)
...
# Reversible binding reaction: Ca + ECM ↔ ECM_Ca
ecs.add_reaction(
    reactants=[ca, ecm],
    products=[ecm_ca],
    k_f=ECM_KF,
