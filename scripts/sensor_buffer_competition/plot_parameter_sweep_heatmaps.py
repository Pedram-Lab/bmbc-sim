...
for i, conc in enumerate(buffer_concs):
    for j, kd in enumerate(buffer_kds):
        # Construct simulation name pattern
        sim_name = f"sensor_buffer_competition_conc{conc}_kd{kd}"
...
X, Y = np.meshgrid(np.arange(len(buffer_kds)), np.arange(len(buffer_concs)))
...
def plot_heatmap(data, ax, title):
    ...
    ax.set_xticks(np.arange(len(buffer_kds)))
    ax.set_yticks(np.arange(len(buffer_concs)))
    ax.set_xticklabels([f"{kd:.0e}" for kd in buffer_kds])
    ax.set_yticklabels([f"{conc:.0e}" for conc in buffer_concs])

    # Add colorbar
    plt.colorbar(im, ax=ax, label='Estimated/Actual Ca²⁺ ratio')

    # Labels and title
    ax.set_xlabel('Buffer Kd (mM)')
    ax.set_ylabel('Buffer concentration (mM)')
    ax.set_title(title)
