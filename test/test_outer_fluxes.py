...
def test_fluxes_from_to_outside(tmp_path, width, visualize=False, skip_assert=False):
    ...
    cell.initialize_species(too_low, 0.5 * mM)
    cell.add_diffusion(too_low, 1 * u.um**2 / u.ms)
    permeability = 10 * u.nm / u.ms * left_membrane.area
    t = transport.Passive(permeability=permeability, outside_concentration=0.7 * mM)
    ...
    cell.initialize_species(too_high, 0.8 * mM)
    ...
    cell.initialize_species(deplete, 0.4 * mM)
    cell.add_diffusion(deplete, 1 * u.um**2 / u.ms)
    t = transport.Active(v_max=50 * u.amol / u.s, km=1 * mM)
    ...
    cell.initialize_species(constant_influx, 0.1 * mM)
