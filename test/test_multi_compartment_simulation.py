...
def test_multi_compartment_dynamics(tmp_path, visualize=False):
    ...
    left.initialize_species(equilibrium, 0.6 * mM)
    right.initialize_species(equilibrium, 1.2 * mM)
    ...
    left.initialize_species(michaelis_menten, 0.9 * mM)
    right.initialize_species(michaelis_menten, 0.9 * mM)
    left.add_diffusion(michaelis_menten, 10 * u.um**2 / u.s)
    right.add_diffusion(michaelis_menten, 10 * u.um**2 / u.s)
    t = transport.Active(v_max=1 * u.amol / u.s, km=0.5 * mM)
    membrane.add_transport(
        species=michaelis_menten, transport=t, source=left, target=right
    )

    # Species with channel transport that creates a discontinuity
    channel_l2r = simulation.add_species("channel_l2r", valence=0)
    left.initialize_species(channel_l2r, 1.1 * mM)
    right.initialize_species(channel_l2r, 0 * mM)
    ...
    left.initialize_species(channel_r2l, 0 * mM)
    right.initialize_species(channel_r2l, 1.3 * mM)
    ...
    def total_substance_sum(species, t):
        return left.sel(species=species).isel(time=t) + right.sel(species=species).isel(
            time=t
        )
    ...
    if visualize:
        ...
        _, ((ax1, ax2), (ax3, ax4)) = plt.subplots(
            2, 2, figsize=(10, 10), sharey=True, gridspec_kw={"wspace": 0}
        )
