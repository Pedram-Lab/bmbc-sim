...
def performance_comparison():
    ...
    for y0, name in zip(test_cases, case_names):
        # Fixed-point method (default)
        start_time = time.time()
        _, y_fp = solver.solve(y0, t_span, dt=0.01, adaptive=True)
        fp_time = time.time() - start_time

        # Newton method
        start_time = time.time()
        _, y_newton = solver.solve_with_method(y0, t_span, dt=0.01,
        ...
        print(
            f"{name:<12} {fp_time:<10.4f} {newton_time:<12.4f} "
            f"{ratio:<8.2f} {solution_diff:<15.2e}"
        )
    ...
    print("\nSummary:")
    print("- Fixed-point method uses adaptive relaxation and stagnation detection")
    print("- No matrix inversions required (more cache-friendly)")
    print("- Better suited for batch processing and FEM applications")
    if total_ratio < 1.0:
    ...
    try:
        start_time = time.time()
        t_fp_stiff, _ = stiff_solver.solve(y0_stiff, (0, 1.0), dt=0.001, adaptive=True)
    ...
    try:
        start_time = time.time()
        t_newton_stiff, _ = stiff_solver.solve_with_method(
...
def test_adaptive_behavior():
    ...
    # Store original method once outside the loop
    original_solve_method = solver.newton_solve

    for tol in tolerances:
        start_time = time.time()

        # Create a closure that captures the tolerance value
        def create_custom_solve(tolerance, original_method):
            def custom_solve(y_old, dt, y_guess=None, max_iter=50, tol_param=tolerance):
                return original_method(y_old, dt, y_guess, max_iter, tol_param)
            return custom_solve

        # Modify the solver to use specific tolerance
        solver.newton_solve = create_custom_solve(tol, original_solve_method)
        ...
        try:
            ...
            print(
                f"Tolerance {tol:.0e}: {solve_time:.4f}s, {len(t)} steps, "
                f"conservation error: {max_conservation_error:.2e}"
            )
        except (ValueError, RuntimeError, AttributeError) as e:
            print(f"Tolerance {tol:.0e}: Failed - {e}")
        finally:
            solver.newton_solve = original_solve_method
